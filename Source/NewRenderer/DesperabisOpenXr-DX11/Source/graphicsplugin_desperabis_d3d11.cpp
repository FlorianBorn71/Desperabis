...
extern ProfilingStats g_profilingStats;
...
struct DesperabisD3D11GraphicsPlugin : public IGraphicsPlugin {
    DesperabisD3D11GraphicsPlugin(const std::shared_ptr<Options>& options, std::shared_ptr<IPlatformPlugin>)
	...
	FUTURE(void) Desp_Init()
	{
		...
		ComPtr<ID3D11RenderTargetView1> d3dRenderTargetView; // both still null here, see ReInitialize
		ComPtr<ID3D11DepthStencilView> d3dDepthStencilView; // both still null here, see ReInitialize
        D3D11_VIEWPORT viewPort;
		UINT numViewports = 1;

		// Store pointers to the Direct3D 11.3 API device and immediate context.
		ThrowIfFailed(
			m_device.As(&d3dDevice)
		);
		ThrowIfFailed(
            m_deviceContext.As(&d3dContext)
		);
        d3dContext->RSGetViewports(&numViewports, &viewPort);
        
		// we need the renderer before Asset Manager...
		...
		{
			Matrix4 projMat;
			float aspectRt = viewPort.Width / viewPort.Height;
    ...
    void RenderView(const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage,
        ...
        ComPtr<ID3D11DepthStencilView> depthStencilView = GetDepthStencilView(colorTexture);

		const GlobalSettings& settings = GlobalSettings::Instance();
        if (g_desperabisScene != nullptr && g_desperabisScene->IsReady())
        {
            Vector4 clearCol = g_desperabisScene->GetClearColor();
            m_clearColor[0] = powf(clearCol.x, settings.gamma);
            m_clearColor[1] = powf(clearCol.y, settings.gamma);
            m_clearColor[2] = powf(clearCol.z, settings.gamma);
        }
        ...
        ThrowIfFailed(
            renderTargetView.As(&d3dRenderTargetView)
        );

		// Hook Desperabis rendering
		if (g_desperabisScene != nullptr && g_desperabisScene->IsReady())
		{
            XrPosef renderPose = layerView.pose;
            if (const GridCellArray* grid = g_desperabisScene->GetGrid())
            {
				renderPose.position.x += g_playerPos.x;
				renderPose.position.z += g_playerPos.z;
                Vector currentPos(renderPose.position.x, renderPose.position.y, renderPose.position.z, 0.f);
                float height = grid->GetHeightAtPos(currentPos) + 55.f *DespToFloat;
				renderPose.position.y += height;
            }

            const XMMATRIX poseView = LoadXrPose(renderPose);

            const XMMATRIX spaceToView = XMMatrixInverse(nullptr, poseView);
            XrMatrix4x4f projectionMatrix;
            XrMatrix4x4f_CreateProjectionFov(&projectionMatrix, GRAPHICS_D3D, layerView.fov, 0.05f, 100.0f);

            g_RendererDX11.ReInitialize(d3dRenderTargetView.Get(), depthStencilView.Get(), viewport);

            Camera* cam = g_RendererDX11.GetOrCreateCamera().get();
            Matrix4 despProj, despView;
            despProj.Load(projectionMatrix.m);
            despView.m_matrix = spaceToView;

            cam->SetProjectionMatrix(despProj);
            cam->SetViewMatrix(despView);

            g_RendererDX11.SetCurrentTime(g_currentTime);
            g_RendererDX11.RenderScene(g_desperabisScene.get(), &g_profilingStats);
...
std::shared_ptr<IGraphicsPlugin> CreateGraphicsPlugin_DesperabisD3D11(const std::shared_ptr<Options>& options,
                                                            std::shared_ptr<IPlatformPlugin> platformPlugin) {
    return std::make_shared<DesperabisD3D11GraphicsPlugin>(options, platformPlugin);
}
