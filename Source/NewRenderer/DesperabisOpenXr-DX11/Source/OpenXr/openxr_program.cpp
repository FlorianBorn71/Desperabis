...
struct OpenXrProgram : IOpenXrProgram {
    ...
    void InitializeActions() {
        ...
        // needs to be called before xrAttachSessionActionSets
		m_desperabisCamera = std::make_shared<DesperabisOpenXrCamera>();
		m_desperabisCamera->Initialize(m_instance, m_session, m_input.actionSet);
    ...
    void CreateVisualizedSpaces() {
        ...
        for (const auto& visualizedSpace : visualizedSpaces) {
            ...
            if (XR_SUCCEEDED(res)) {
                if (referenceSpaceCreateInfo.referenceSpaceType == XR_REFERENCE_SPACE_TYPE_LOCAL) {
                    m_playerLocalSpace = space;
                }
    ...
    void MoveDesperabisPlayer(XrTime currentTime)
    {
        /*
		XrSpaceLocation spaceLocation = { XR_TYPE_SPACE_LOCATION };
		xrLocateSpace(m_playerLocalSpace, m_appSpace, currentTime, &spaceLocation);

		if (spaceLocation.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) {
			XrVector3f forwardMovement = { 0.0f, 0.0f, -0.1f }; // Move forward by 0.1 units

			// Move the player in the direction they are currently facing
			spaceLocation.pose.position.x += forwardMovement.x;
			spaceLocation.pose.position.y += forwardMovement.y;
			spaceLocation.pose.position.z += forwardMovement.z;
		}

        */
        m_desperabisCamera->Update(m_appSpace, currentTime);
    }
    ...
    void RenderFrame() override {
        ...
        MoveDesperabisPlayer(frameState.predictedDisplayTime);
   ...
   private:
    ...
    XrSpace m_playerLocalSpace{XR_NULL_HANDLE};
    ...
    std::shared_ptr<DesperabisOpenXrCamera> m_desperabisCamera;
};
